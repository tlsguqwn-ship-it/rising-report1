        ...
        <table className="w-full text-left border-collapse table-fixed">
          <thead>
            <tr className={`h-[8mm] border-b ${cardBorder} ${labelText}`}>
              ...
              <th className="pl-0 pr-2 text-[11px] font-bold uppercase tracking-tight" style={{ width: isPreMarket ? '28%' : '22%' }}>
                {isPreMarket ? (
              ...
              <th className={`${isPreMarket ? 'pl-4' : 'pl-8'} pr-3 text-[11px] font-bold uppercase tracking-tight`} style={{ width: isPreMarket ? '50%' : '56%' }}>{isPreMarket ? '투자 포인트' : '등락 사유 및 분석'}</th>
            </tr>
          </thead>
          <tbody className={`divide-y ${isDark ? 'divide-[#1a1a24]' : 'divide-slate-50'}`}>
            {data.featuredStocks.map((stock, idx) => {
              ...
              <tr key={stock.id || idx} data-arr="featuredStocks" className={`${isDark ? 'hover:bg-[#22222e]' : 'hover:bg-slate-50'} transition-colors group/row relative`}>
                ...
                <td className={`py-2 pl-0 border-r ${isDark ? 'border-[#1a1a24]' : 'border-slate-50'} align-middle`} style={{ width: isPreMarket ? '28%' : '22%' }}>
                  {isPreMarket ? (
                ...
                <td className={`${isPreMarket ? 'pl-4' : 'pl-8'} pr-3 py-2 text-[15px] font-medium ${subText} leading-[1.5] align-middle`} style={{ width: isPreMarket ? '50%' : '56%' }}>
